...
export async function GET(req: Request) {
  ...
  let query = supabaseAdmin
    .from('operation_logs')
    .select('id, actor_email, actor_role, school_id, school_name, action, detail, metadata, created_at')
    .order('created_at', { ascending: false })
    .limit(limit)

  if (search) query = query.or(`actor_email.ilike.%${search}%,school_name.ilike.%${search}%,detail.ilike.%${search}%`)
